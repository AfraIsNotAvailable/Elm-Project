...
var $author$project$Model$PostIds$advance = function (_v0) {
	...
	return A2(
		$elm$core$Maybe$map,
		function (newCursor) {
			var newId = $author$project$Cursor$current(newCursor);
			return _Utils_Tuple2(
				newId,
...
var $author$project$Model$PostIds$decode = function () {
	var listDecoder = $elm$json$Json$Decode$list($elm$json$Json$Decode$int);
	return A2($elm$json$Json$Decode$map, $author$project$Model$PostIds$fromList, listDecoder);
}();
							...
							return _Utils_Tuple2(
								$author$project$Model$LoadedPosts(
									_Utils_update(
										state,
										{
											config: A2($author$project$Model$PostsConfig$applyChanges, change, state.config)
										})),
								$author$project$Effect$NoEffect);
						} else {
							break _v1$4;
...
var $author$project$Util$Time$durationBetween = F2(
	function (t1, t2) {
		var t2Millis = $elm$time$Time$posixToMillis(t2);
		var t1Millis = $elm$time$Time$posixToMillis(t1);
		var msPerSecond = 1000;
		var msPerMinute = msPerSecond * 60;
		var msPerHour = msPerMinute * 60;
		var msPerDay = msPerHour * 24;
		var diffMillis = t2Millis - t1Millis;
		if (diffMillis <= 0) {
			return $elm$core$Maybe$Nothing;
		} else {
			var remainingAfterDays = A2($elm$core$Basics$modBy, msPerDay, diffMillis);
			var remainingAfterHours = A2($elm$core$Basics$modBy, msPerHour, remainingAfterDays);
			var remainingAfterMinutes = A2($elm$core$Basics$modBy, msPerMinute, remainingAfterHours);
			var seconds = (remainingAfterMinutes / msPerSecond) | 0;
			var minutes = (remainingAfterHours / msPerMinute) | 0;
			var hours = (remainingAfterDays / msPerHour) | 0;
			var days = (diffMillis / msPerDay) | 0;
			return $elm$core$Maybe$Just(
...
var $author$project$Util$Time$formatDuration = function (duration) {
	var formatUnit = F2(
		function (count, unitName) {
			return (count <= 0) ? $elm$core$Maybe$Nothing : ((count === 1) ? $elm$core$Maybe$Just('1 ' + unitName) : $elm$core$Maybe$Just(
				$elm$core$String$fromInt(count) + (' ' + (unitName + 's'))));
		});
	var parts = A2(
		...
		_List_fromArray(
			[
				A2(formatUnit, duration.days, 'day'),
				A2(formatUnit, duration.hours, 'hour'),
				A2(formatUnit, duration.minutes, 'minute'),
				A2(formatUnit, duration.seconds, 'second')
			]));
...
var $author$project$View$Posts$postRow = F2(
	function (currentTime, post) {
		...
		var postedDateStr = A2($author$project$Util$Time$formatTime, $elm$time$Time$utc, post.time);
		var duration = A2($author$project$Util$Time$durationBetween, post.time, currentTime);
		var durationStr = A2(
			$elm$core$Maybe$withDefault,
			'time error',
			A2($elm$core$Maybe$map, $author$project$Util$Time$formatDuration, duration));
		var timeDisplay = postedDateStr + (' (' + (durationStr + ')'));
		return A2(
			$elm$html$Html$tr,
			_List_Nil,
			_List_fromArray(
...
var $author$project$View$Posts$postsPerPageInput = function (config) {
	...
	var onChange = function (s) {
		...
		if (_v0.$ === 'Just') {
			...
			return $author$project$Model$ConfigChanged(
				$author$project$Model$PostsConfig$ChangePostsToShow(n));
		} else {
			return $author$project$Model$ConfigChanged(
				$author$project$Model$PostsConfig$ChangePostsToShow(10));
		}
	};
	var current = $elm$core$String$fromInt(config.postsToShow);
	var postsCountOption = function (num) {
		var s = $elm$core$String$fromInt(num);
		return A2(
	...
	return A2(
		...
		A2($elm$core$List$map, postsCountOption, options));
};
...
var $author$project$View$Posts$showJobsInput = function (config) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
...
var $author$project$View$Posts$showTextOnlyInput = function (config) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
...
var $author$project$View$Posts$sortByInput = function (config) {
	var onChange = function (s) {
		var _v0 = $author$project$Model$PostsConfig$sortFromString(s);
		if (_v0.$ === 'Just') {
			var sortBy = _v0.a;
			return $author$project$Model$ConfigChanged(
	...
	var currentSortStr = $author$project$Model$PostsConfig$sortToString(config.sortBy);
	var sortOption = function (sort) {
		var s = $author$project$Model$PostsConfig$sortToString(sort);
		return A2(
			$elm$html$Html$option,
			_List_fromArray(
				[
	...
	return A2(
		...
		A2($elm$core$List$map, sortOption, $author$project$Model$PostsConfig$sortOptions));
};
var $author$project$View$Posts$postsConfigView = function (config) {
	return A2(
		...
		_List_fromArray(
			[
				$author$project$View$Posts$postsPerPageInput(config),
				$author$project$View$Posts$sortByInput(config),
				$author$project$View$Posts$showJobsInput(config),
