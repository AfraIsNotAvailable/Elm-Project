...
-- formatUnit : Int -> String -> Maybe String
-- formatUnit count unitName =
--     if count <= 0 then
--         Nothing
--     else if count == 1 then
--         Just ("1 " ++ unitName)
--     else
--         Just (String.fromInt count ++ " " ++ unitName ++ "s")
...
durationBetween t1 t2 =
    ...
    if diffMillis <= 0 then
        ...
        let
            ...
            remainingAfterDays =
                modBy msPerDay diffMillis

            hours =
                remainingAfterDays // msPerHour

            remainingAfterHours =
                modBy msPerHour remainingAfterDays

            minutes =
                remainingAfterHours // msPerMinute

            remainingAfterMinutes =
                modBy msPerMinute remainingAfterHours

            seconds =
                remainingAfterMinutes // msPerSecond
        in
        Just
            { seconds = seconds
            , minutes = minutes
